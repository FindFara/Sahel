...
const birthdayPage = document.querySelector('[data-birthday-page]');
const loginForm = document.querySelector('[data-login-form]');
const loginInput = document.querySelector('#birthdayPassword');
...
function unlockBirthdayPage() {
  ...
  if (birthdayPage) {
    birthdayPage.classList.remove('is-locked');
    birthdayPage.removeAttribute('aria-hidden');
  }
...
function setupLoginGate() {
  ...
  loginForm.addEventListener('submit', (event) => {
    ...
    if (loginInput.value.trim().toLowerCase() === 'haku') {
      if (loginError) {
    ...
    if (loginError) {
      loginError.textContent = 'اسم درست نیست؛ دوباره امتحان کن 🐾';
    }
